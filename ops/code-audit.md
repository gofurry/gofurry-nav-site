...
整体结论：首版架构清晰，Agent 与 Center 的边界也比较克制；HMAC 上报、时间窗口校验、cookie 登录、SQL 参数化、collector timeout、spool 权限等基础安全面已经具备。当前没有发现 P0 级问题。原审计发现的 1 个 P1、3 个 P2、4 个 P3 均已在 2026-05-19 完成修复并补充测试，当前没有未关闭 finding。
...
- P1-001 Fixed：Center 增加登录与写入型入口 limiter，设置 2 MiB body limit，配置校验拒绝短值和占位弱密钥，passcode 改为固定时间比较，session token 加入随机 nonce。
- P2-001 Fixed：Repository ingest 返回本次写入后的 service failure count，HTTP 告警只有连续失败数达到 `http_failure_threshold` 后才创建或刷新，成功样本会 resolve。
- P2-002 Fixed：raw sample 表补齐 `node_id + reported_at`、`reported_at`、`received_at` 相关索引，覆盖 network、docker、http、service、cert，以及 system/disk/heartbeat 的全局趋势与清理索引。
- P2-003 Fixed：Agent ingest 增加 payload 业务边界校验，包括数组数量、身份字段长度、URL 长度、状态枚举和错误消息截断。
- P3-001 Fixed：session token 改为 `expires:nonce:signature`，同一秒多次登录也会生成不同 token；按当前轻量设计不引入服务端 session 存储。
- P3-002 Fixed：Agent Postgres collector 改为单次 `pgx.Connect`，保留 `SELECT 1` 和元数据查询，减少每次采样创建连接池的抖动。
- P3-003 Fixed：TLS 证书 collector 改用 `tls.Dialer.DialContext`，拨号和 TLS 握手受采集 context 控制。
- P3-004 Fixed：spool replay 对单行设置 2 MiB 上限，并在超大或损坏 JSONL 文件出现时隔离到 `.bad`，继续处理后续正常缓存。
