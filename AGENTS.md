...
Collaboration Center stores ideas and its single text board only in GFA; its GFG/GFN access is read-only. All GFA mutations use version checks and transactional Audit (except Board geometry-only Audit). Formal creation remains GameAPI/NavAPI; preserve successful creation if linking fails. See `docs/collaboration-center.md`.
