...
All five Go binaries use Cobra with an explicit `serve --config <file>` foreground lifecycle. Linux/systemd registration is generated by each binary's `install --config <file>` command; install enables but never starts, and no static production unit is canonical.
