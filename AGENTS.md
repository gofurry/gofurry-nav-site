...
- Runtime YAML uses `gofurry_app` for `gfn`, `gfg`, and `gfa`, plus the Redis `gofurry_app` ACL username.
- Goose alone uses `gofurry_migrator`; never use it as an application account.
- GUI and manual read-only inspection use `gofurry_readonly`.
- `gofurry_admin` is for infrastructure administration only and must not appear in ordinary local application configuration or the root `.env`.
- Never commit `.env`, local `server.yaml`, credentials, or real Tailnet addresses. See `docs/development.md` and `docs/operations/dev-infrastructure.md`.
...
Collaboration Center stores ideas and its single shared canvas only in GFA; its GFG/GFN access is read-only. All GFA mutations use version checks and transactional Audit (except Board geometry-only Audit). Formal creation remains GameAPI/NavAPI; preserve successful creation if linking fails. See `docs/collaboration-center.md`.
