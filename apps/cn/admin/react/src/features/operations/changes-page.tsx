...
function EntityChangesPanel() {
  ...
  return <div className="grid gap-5"><Section title="选择实体" description="按网站或游戏查看可重现的语义变化历史。"><div className="grid gap-3 md:grid-cols-[12rem_1fr]"><FilterField label="Domain"><Select value={domain} onValueChange={(value) => { setDomain(value as 'game' | 'nav'); setEntity(null) }} options={[{ value: 'nav', label: '网站' }, { value: 'game', label: '游戏' }]} /></FilterField><RemoteSelect endpoint={domain === 'game' ? '/api/v1/options/games' : '/api/v1/options/sites'} value={entity} onChange={setEntity} placeholder={domain === 'game' ? '搜索游戏…' : '搜索网站…'} /></div></Section>{entity ? <ChangeTable key={`${domain}-${entity.id}`} domain={domain} entityID={Number(entity.id)} /> : <Section><div className="grid min-h-40 place-items-center text-sm text-muted-foreground"><FileClock className="mb-2 size-8" />请先选择实体。</div></Section>}</div>
}
