...
import { FilterField, JsonBlock, Kpi, KpiGrid, OperationTabs, OperationsChart, RemotePicker, type OperationTab } from '../../components/admin/operations'
...
function CollectionOverviewPanel() {
  ...
  const charts = useQuery({ queryKey: ['collection', 'charts'], queryFn: () => getJSON<CollectionChartPoint[]>('/api/v1/collection/charts/outcomes?window=7d') })
...
function RunningPanel() {
  ...
  const cancel = useMutation({ mutationFn: (job: CollectionJob) => sendJSON(`/api/v1/collection/jobs/${job.domain}/${job.id}/cancel`, 'POST'), onSuccess: async () => { await client.invalidateQueries({ queryKey: ['collection'] }); toast('已请求取消任务') }, onError: (error) => toast(errorMessage(error), 'danger') })
  const columns = useMemo<AdminColumn<CollectionJob>[]>(() => [
    ...
    { key: '_control', header: '', sortable: false, render: (row) => access.control ? <Button size="sm" variant="secondary" disabled={cancel.isPending} onClick={(event) => { event.stopPropagation(); cancel.mutate(row) }}><Ban className="size-3.5" />取消</Button> : null },
  ], [access.control, cancel])
  return <DataTable data={query.data ?? []} columns={columns} total={query.data?.length ?? 0} page={1} pageSize={100} search="" onSearchChange={() => undefined} onPageChange={() => undefined} onPageSizeChange={() => undefined} searchable={false} loading={query.isLoading} error={query.error?.message} onRetry={() => void query.refetch()} />
}
...
function RunHistoryPanel() {
  ...
  const domain = params.get('domain') ?? ''; const status = params.get('status') ?? ''; const trigger = params.get('trigger') ?? ''
  const [selected, setSelected] = useState<CollectionRun | null>(null)
  const query = useQuery({ queryKey: ['collection', 'runs', page, pageSize, domain, status, trigger], queryFn: () => getJSON<PageResult<CollectionRun>>(`/api/v1/collection/runs?${queryString({ page, page_size: pageSize, domain, status, trigger })}`) })
