...
export function BatchIdeaDialog({ close }: { close: () => void }) {
  ...
  </>}><div className="grid gap-4"><Select ariaLabel="批量类型" options={kindOptions} value={kind} disabled={check.isPending || create.isPending} onValueChange={(value) => { setKind(value as Kind); resetPreview() }} /><Textarea aria-label="粘贴内容" placeholder={batchExamples[kind]} rows={7} value={text} disabled={check.isPending || create.isPending} onChange={(e) => { setText(e.target.value); resetPreview() }} />
