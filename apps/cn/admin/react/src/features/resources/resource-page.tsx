...
function ResourceEditor({ definition, id, open, onOpenChange }: { definition: ResourceDefinition; id: number | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  const client = useQueryClient()
  const { toast } = useToast()
  const [operationError, setOperationError] = useState('')
  const detail = useQuery({ queryKey: ['resource-detail', definition.key, id], queryFn: () => getJSON<ResourceRecord>(`${definition.detailEndpoint}/${id}`), enabled: open && id !== null })
  ...
  const mutation = useMutation({ mutationFn: (payload: ResourceRecord) => id === null ? sendJSON(definition.detailEndpoint, 'POST', payload) : sendJSON(`${definition.detailEndpoint}/${id}`, 'PUT', payload), onSuccess: async () => {
...
export function ResourcePage() {
  ...
  const deleteMutation = useMutation({ mutationFn: () => sendJSON(`${definition!.detailEndpoint}/${deleting?.id}`, 'DELETE'), onSuccess: async () => { await client.invalidateQueries({ queryKey: ['resource-list', definition?.key] }); toast(`${definition?.title}已删除`); setDeleting(null) }, onError: (error) => toast(errorMessage(error), 'danger') })
