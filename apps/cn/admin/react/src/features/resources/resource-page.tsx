...
function ResourceEditor({ definition, id, open, onOpenChange }: { definition: ResourceDefinition; id: string | null; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const mutation = useMutation({ mutationFn: (payload: ResourceRecord) => id === null ? sendJSON(definition.detailEndpoint, 'POST', payload) : sendJSON(resourceEndpoint(definition.detailEndpoint, id), 'PUT', payload), onSuccess: async () => {
    ...
    if (definition.archive) { await client.invalidateQueries({ queryKey: ['options'] }); await client.invalidateQueries({ queryKey: ['remote-picker'] }) }
...
export function ResourcePage({ section, resource }: { section: ResourceSection; resource?: string }) {
  ...
  const deleteMutation = useMutation({ mutationFn: () => sendJSON(resourceEndpoint(definition!.detailEndpoint, resourceRecordID(deleting!)) + (definition?.archive && deleting?.archived_at ? '/restore' : ''), definition?.archive && deleting?.archived_at ? 'POST' : 'DELETE'), onSuccess: async () => { await client.invalidateQueries({ queryKey: ['resource-list', definition?.key] }); await client.invalidateQueries({ queryKey: ['options'] }); await client.invalidateQueries({ queryKey: ['remote-picker'] }); toast(`${definition?.title}${definition?.archive ? (deleting?.archived_at ? '已恢复' : '已归档') : '已删除'}`); setDeleting(null) }, onError: (error) => toast(errorMessage(error), 'danger') })
  ...
  return <PageLayout>
    ...
    <DataTable data={query.data?.list ?? []} columns={definition.key === 'site-groups' ? [...columns, { key: 'curation', header: '首页展示', render: (row) => <Link className="text-primary hover:underline" to={`/nav/site-groups/${resourceRecordID(row)}/curation`} onClick={(event) => event.stopPropagation()}>首页编排</Link> }] : definition.archive ? [...columns, { key: 'archive', header: '归档状态', render: (row) => canWrite && <Button variant="secondary" onClick={(event) => { event.stopPropagation(); setDeleting(row) }}>{row.archived_at ? '恢复' : '归档'}</Button> }] : columns} total={query.data?.total ?? 0} page={page} pageSize={pageSize} search={search} onSearchChange={(value) => { const next = new URLSearchParams(params); next.set('page', '1'); if (value) next.set('search', value); else next.delete('search'); setParams(next, { replace: true }) }} onPageChange={(value) => changeParam('page', String(value))} onPageSizeChange={(value) => { const next = new URLSearchParams(params); next.set('page', '1'); next.set('page_size', String(value)); setParams(next, { replace: true }) }} onEdit={canWrite ? (row) => setEditor({ open: true, id: resourceRecordID(row) }) : undefined} onDelete={canWrite && !definition.archive ? setDeleting : undefined} onRowClick={canWrite ? (row) => setEditor({ open: true, id: resourceRecordID(row) }) : undefined} loading={query.isLoading} error={query.error?.message} onRetry={() => void query.refetch()} />
    <ResourceEditor definition={definition} id={editor.id} open={editor.open} onOpenChange={(open) => setEditor((current) => ({ ...current, open }))} />
    <ConfirmAction confirmLabel={definition.archive ? (deleting?.archived_at ? '确认恢复' : '确认归档') : '确认删除'} open={Boolean(deleting)} onOpenChange={(open) => { if (!open) setDeleting(null) }} title={`${definition.archive ? (deleting?.archived_at ? '恢复' : '归档') : '删除'}${definition.title}`} description={definition.archive ? '归档保留标识与历史引用。仍在使用的标签或类别不能归档。' : `确定删除记录 #${deleting?.id ?? ''} 吗？`} busy={deleteMutation.isPending} onConfirm={() => deleteMutation.mutate()} />
  </PageLayout>
}
