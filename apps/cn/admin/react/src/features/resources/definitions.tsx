...
export const resourceDefinitions: ResourceDefinition<ResourceRecord>[] = [
  ...
  {
    key: 'tag-categories', section: 'game', title: '标签类别', archive: true, description: '维护标签类别的名称和稳定标识；已使用的标签需先从游戏移除才能归档。',
    listEndpoint: '/api/v1/game/tag-categories', detailEndpoint: '/api/v1/game/tag-categories',
    columns: [{ key: 'id', label: 'ID', hidden: true }, { key: 'code', label: 'Code' }, { key: 'name', label: '名称' }, { key: 'name_en', label: '英文名称' }, { key: 'archived_at', label: '状态', format: (value: unknown) => <StatusBadge tone={value ? 'neutral' : 'success'}>{value ? '已归档' : '有效'}</StatusBadge> }],
    fields: [{ key: 'code', label: 'Code', type: 'text', createOnly: true, help: '创建后不可修改，使用小写字母、数字和连字符。', section: '标识' }, { key: 'name', label: '中文名称', type: 'text', section: '基本内容' }, { key: 'name_en', label: '英文名称', type: 'text' }, { key: 'info', label: '中文简介', type: 'textarea' }, { key: 'info_en', label: '英文简介', type: 'textarea' }, { key: 'sort_order', label: '展示顺序', type: 'number' }],
    defaults: { code: '', name: '', name_en: '', info: '', info_en: '', sort_order: 0 },
    schema: z.object({ code: z.string().min(1).max(64).regex(/^[a-z0-9]+(-[a-z0-9]+)*$/, '请输入小写 kebab-case 标识'), name: z.string().trim().min(1).max(255), name_en: z.string().trim().min(1).max(255), info: z.string().max(255), info_en: z.string().max(255), sort_order: z.coerce.number().int() }),
  },
  {
    key: 'tags', section: 'game', title: '标签', archive: true, description: '维护标签的名称和稳定标识；已使用的标签需先从游戏移除才能归档。',
    listEndpoint: '/api/v1/game/tags', detailEndpoint: '/api/v1/game/tags',
    columns: [{ key: 'id', label: 'ID', hidden: true }, { key: 'code', label: 'Code' }, { key: 'name', label: '名称' }, { key: 'name_en', label: '英文名称' }, { key: 'archived_at', label: '状态', format: (value: unknown) => <StatusBadge tone={value ? 'neutral' : 'success'}>{value ? '已归档' : '有效'}</StatusBadge> }],
    fields: [{ key: 'code', label: 'Code', type: 'text', createOnly: true, help: '创建后不可修改，使用小写字母、数字和连字符。', section: '标识' }, { key: 'name', label: '中文名称', type: 'text', section: '基本内容' }, { key: 'name_en', label: '英文名称', type: 'text' }, { key: 'info', label: '中文简介', type: 'textarea' }, { key: 'info_en', label: '英文简介', type: 'textarea' }, { key: 'category_id', label: '所属类别', type: 'remote-select', optionEndpoint: '/api/v1/options/tag-categories' }],
    defaults: { code: '', name: '', name_en: '', info: '', info_en: '', category_id: 0 },
    schema: z.object({ code: z.string().min(1).max(64).regex(/^[a-z0-9]+(-[a-z0-9]+)*$/, '请输入小写 kebab-case 标识'), name: z.string().trim().min(1).max(255), name_en: z.string().trim().min(1).max(255), info: z.string().max(255), info_en: z.string().max(255), category_id: z.coerce.number().int().positive('请选择类别') }),
  },
  {
