...
export function AuditPage() {
  ...
  const toolbar = <><FilterField label="历史角色"><NativeSelect value={role} onChange={(event) => resetPage(setRole)(event.target.value)}><option value="">全部角色</option><option value="owner">Owner</option><option value="developer">Developer</option><option value="operator">Operator</option><option value="system">System</option></NativeSelect></FilterField><FilterField label="Action"><input className="h-9 rounded-md border bg-surface px-3 text-sm" value={action} onChange={(event) => resetPage(setAction)(event.target.value)} placeholder="account.role" /></FilterField><FilterField label="Resource"><input className="h-9 rounded-md border bg-surface px-3 text-sm" value={resource} onChange={(event) => resetPage(setResource)(event.target.value)} placeholder="gfa_admin_account" /></FilterField><FilterField label="开始"><input className="h-9 rounded-md border bg-surface px-3 text-sm" type="date" value={from} onChange={(event) => resetPage(setFrom)(event.target.value)} /></FilterField><FilterField label="结束"><input className="h-9 rounded-md border bg-surface px-3 text-sm" type="date" value={through} onChange={(event) => resetPage(setThrough)(event.target.value)} /></FilterField></>
