...
export function GameWorkspacePage() {
  const { id } = useParams()
  const navigate = useNavigate()
  if (id === 'new') return <PageLayout><PageHeader title="新增游戏" description="可先填写 Steam AppID 获取预填内容，再进入完整 Game Workspace。" actions={<Button variant="secondary" onClick={() => navigate('/game/games')}><ArrowLeft className="size-4" />返回列表</Button>} /><GameContentForm game={emptyGame()} creating /></PageLayout>
