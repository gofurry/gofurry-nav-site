...
function GameClassificationForm({ workspace }: { workspace: GameWorkspace }) {
  ...
  return <Section title="分类与展示" description="主标签、副标签与完整标签集合在同一工作流维护。"><form className="grid gap-6" onSubmit={form.handleSubmit((values) => { setOperationError(''); mutation.mutate(values) })}>{operationError && <Alert tone="danger">{operationError}</Alert>}<FormSection title="主要分类"><div className="grid gap-4 md:grid-cols-3"><FormField label="主要标签"><select className="h-9 rounded-md border bg-surface px-3 text-sm" {...form.register('primary_tag')}><option value={0}>无</option>{tagOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></FormField><FormField label="次要标签"><select className="h-9 rounded-md border bg-surface px-3 text-sm" {...form.register('secondary_tag')}><option value={0}>无</option>{tagOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</select></FormField><FormField label="展示权重"><Input type="number" {...form.register('weight')} /></FormField></div></FormSection><FormSection title="全部标签" description="保存后一次替换 Game 的 Tag Map 集合。"><TagMultiSelect options={tagOptions} selected={form.watch('tag_ids')} onChange={(value) => form.setValue('tag_ids', value, { shouldDirty: true })} /></FormSection><div className="flex justify-end"><Button disabled={mutation.isPending || !form.formState.isDirty}>{mutation.isPending && <LoaderCircle className="size-4 animate-spin" />}保存分类与展示</Button></div></form></Section>
}
