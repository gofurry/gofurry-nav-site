...
export function ConfirmAction({ open, onOpenChange, title, description, busy, onConfirm, confirmLabel = '确认删除', variant = 'danger', children }: {
  open: boolean; onOpenChange: (open: boolean) => void; title: string; description: string
  busy?: boolean; onConfirm: () => void; confirmLabel?: string; variant?: 'primary' | 'danger'; children?: ReactNode
}) {
  ...
  </>}><p className="text-sm text-muted-foreground">此操作会立即提交到服务器，请确认目标无误。</p>{children}</Dialog>
}
