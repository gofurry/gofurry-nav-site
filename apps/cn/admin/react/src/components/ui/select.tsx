...
export function Select({ value, onValueChange, options, placeholder = '请选择', disabled, name, className, positionerClassName, popupClassName, ariaLabel }: {
  ...
  return <BaseSelect.Root value={value ?? ''} onValueChange={(next) => next !== null && onValueChange(next)} disabled={disabled} name={name}>
    ...
    <BaseSelect.Portal>
      <BaseSelect.Positioner className={cn('z-[80] outline-none', positionerClassName)} side="bottom" align="start" alignItemWithTrigger={false} sideOffset={4}>
        <BaseSelect.Popup className={cn('max-h-72 min-w-[var(--anchor-width)] overflow-auto rounded-md border bg-surface p-1 shadow-xl', popupClassName)}>
