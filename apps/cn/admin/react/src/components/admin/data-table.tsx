...
export function DataTable<T extends object>({ data, columns, total, page, pageSize, search, onSearchChange, onPageChange, onPageSizeChange, onRowClick, onEdit, onDelete, loading, error, onRetry, searchable = true, searchPlaceholder = '搜索…（按 / 聚焦）', toolbar }: {
  ...
  return <div className="grid gap-3">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex min-w-0 flex-1 flex-wrap items-center gap-2">{searchable && <div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input ref={searchRef} className="pl-9" value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder={searchPlaceholder} aria-label="搜索列表" /></div>}{toolbar}</div>
