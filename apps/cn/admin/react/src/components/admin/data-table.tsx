...
export function DataTable<T extends object>({ data, columns, total, page, pageSize, search, onSearchChange, onPageChange, onPageSizeChange, onRowClick, onEdit, onDelete, loading, error, onRetry }: {
  ...
  return <div className="grid gap-3">
    <div className="flex items-center justify-between gap-3">
      <div className="relative w-full max-w-md"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input ref={searchRef} className="pl-9" value={search} onChange={(event) => onSearchChange(event.target.value)} placeholder="搜索…（按 / 聚焦）" aria-label="搜索列表" /></div>
    ...
    {loading ? <LoadingState /> : error ? <ErrorState message={error} onRetry={onRetry} /> : data.length === 0 ? <EmptyState /> : <div className="overflow-hidden rounded-md border bg-surface"><table className="w-full border-collapse text-sm"><thead className="bg-surface-muted text-left text-xs text-muted-foreground">{table.getHeaderGroups().map((group) => <tr key={group.id}>{group.headers.map((header) => <th key={header.id} className="h-10 border-b px-3 font-medium"><button type="button" className="inline-flex items-center gap-1" disabled={!header.column.getCanSort()} onClick={header.column.getToggleSortingHandler()}>{flexRender(header.column.columnDef.header, header.getContext())}{header.column.getCanSort() && <ChevronsUpDown className="size-3" />}</button></th>)}</tr>)}</thead><tbody>{table.getRowModel().rows.map((row) => <tr key={row.id} className="border-b last:border-0 hover:bg-surface-muted/70" onClick={() => onRowClick?.(row.original)}>{row.getVisibleCells().map((cell) => <td key={cell.id} className="h-12 px-3" onClick={cell.column.id === '_actions' ? (event) => event.stopPropagation() : undefined}>{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>)}</tr>)}</tbody></table></div>}
