...
export function DataTable<T extends object>({ data, columns, total, page, pageSize, search, onSearchChange, onPageChange, onPageSizeChange, onRowClick, onEdit, onDelete, loading, error, onRetry, searchable = true, searchPlaceholder = '搜索…（按 / 聚焦）', toolbar, title, headerActions }: {
  ...
  return <div className="grid gap-3">
    ...
    <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-muted-foreground"><span>显示 {total === 0 ? 0 : (page - 1) * pageSize + 1}–{Math.min(page * pageSize, total)}，共 {total} 条</span><div className="flex items-center gap-2">{onPageSizeChange ? <><span>每页</span><Select className="w-20" value={String(pageSize)} onValueChange={(value) => onPageSizeChange(Number(value))} options={[20, 50, 100].map((value) => ({ value: String(value), label: String(value) }))} ariaLabel="每页条数" /></> : <span>每页 {pageSize} 条</span>}<Button variant="secondary" size="icon" aria-label="上一页" disabled={page <= 1} onClick={() => onPageChange(page - 1)}><ChevronLeft className="size-4" /></Button><span>{page} / {pageCount}</span><Button variant="secondary" size="icon" aria-label="下一页" disabled={page >= pageCount} onClick={() => onPageChange(page + 1)}><ChevronRight className="size-4" /></Button></div></div>
  </div>
}
