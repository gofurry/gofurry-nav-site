...
export function HistoryPanel({ domain, entityId }: { domain: 'nav' | 'game'; entityId: number }) {
  ...
  if (!enabled) return <Alert tone="warning">当前账号没有 <TechnicalLabel>changes.read</TechnicalLabel>，无法读取变化事件。</Alert>
  if (query.isLoading) return <LoadingState label="正在加载变化记录…" />
  if (query.error) return <Alert tone="danger">{query.error.message}</Alert>
  if (!query.data?.list.length) return <EmptyState title="暂无可用历史" message="当前实体没有已物化的变化事件。" />
...
export function DataCenterPanel({ title, children, to }: { title: string; children: ReactNode; to: string }) {
  return <div className="rounded-lg border bg-surface p-6"><StatusBadge tone="info">React 数据中心</StatusBadge><h2 className="mt-3 text-lg font-semibold">{title}</h2><div className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{children}</div><Link to={to} className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-primary hover:underline">打开数据中心<ExternalLink className="size-3.5" /></Link></div>
}
