...
import { Gamepad2, Layers3, Search, Tags, TextSearch } from 'lucide-react'
...
type SearchGroup = { title: string; icon: typeof Search; items: Array<OptionItem & { href: string }> }
...
async function searchEntities(keyword: string): Promise<SearchGroup[]> {
  ...
  return [
    { title: '网站', icon: TextSearch, items: sites.list.map((item) => ({ ...item, href: `/nav/sites/${item.id}` })) },
    { title: '游戏', icon: Gamepad2, items: games.list.map((item) => ({ ...item, href: `/game/games/${item.id}` })) },
    { title: '标签', icon: Tags, items: tags.list.map((item) => ({ ...item, href: `/game/tags?search=${encodeURIComponent(item.label)}` })) },
    { title: '网站分组', icon: Layers3, items: groups.list.map((item) => ({ ...item, href: `/nav/site-groups?search=${encodeURIComponent(item.label)}` })) },
  ]
}
...
export function GlobalSearch({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return <Dialog open={open} onOpenChange={handleOpenChange} title="全局搜索" description="搜索网站、游戏、标签与网站分组">
    <div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input autoFocus className="h-11 pl-9" value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="输入至少两个字符…" /></div>
