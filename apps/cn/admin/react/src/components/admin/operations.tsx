...
export function operationLineSeries(points: Array<{ value: number | null }>, primary: string) {
...
export function OperationsChart({ points, unit, label, loading, error }: { points: Array<{ label: string; value: number | null }>; unit: string; label: string; loading?: boolean; error?: string }) {
  ...
  useEffect(() => {
    ...
    chart.setOption({
      ...
      series: [operationLineSeries(points, primary)],
    })
...
export function RemoteSelect({ endpoint, value, onChange, placeholder = '搜索并选择…', disabled }: { endpoint: string; value: OptionItem | string | null; onChange: (value: OptionItem | null) => void; placeholder?: string; disabled?: boolean }) {
  ...
  const options = query.data?.list ?? emptyOptions
  ...
  const choose = (option: OptionItem) => { onChange(option); setInputValue(option.label); setSearch(option.label); setOpen(false) }
  const handleKeyDown = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'ArrowDown') { event.preventDefault(); setActiveIndex((current) => open ? Math.min(options.length - 1, current + 1) : 0); setOpen(true); return }
    if (event.key === 'ArrowUp') { event.preventDefault(); setActiveIndex((current) => open ? Math.max(0, current - 1) : Math.max(0, options.length - 1)); setOpen(true); return }
    if (event.key === 'Enter' && open && options[activeIndex]) { event.preventDefault(); choose(options[activeIndex]); return }
    if (event.key === 'Escape') { setOpen(false); setInputValue(selected?.label ?? '') }
  }
  return <div className="relative">
    <Search className="pointer-events-none absolute left-3 top-[18px] z-10 size-4 -translate-y-1/2 text-muted-foreground" />
    <input role="combobox" aria-autocomplete="list" aria-expanded={open} aria-controls={listID} aria-activedescendant={open && options[activeIndex] ? `${listID}-${options[activeIndex].id}` : undefined} className="h-9 w-full rounded-md border bg-surface pl-9 pr-9 text-sm outline-none placeholder:text-muted-foreground focus:ring-2 focus:ring-ring disabled:opacity-60" value={displayedInputValue} disabled={disabled} placeholder={placeholder} onFocus={(event) => { setInputValue(selected?.label ?? ''); setOpen(true); event.currentTarget.select() }} onBlur={() => window.setTimeout(() => setOpen(false), 0)} onChange={(event) => { setInputValue(event.target.value); setSearch(event.target.value); setActiveIndex(0); setOpen(true) }} onKeyDown={handleKeyDown} />
