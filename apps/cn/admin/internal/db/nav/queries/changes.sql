-- name: AdminListNavChangeRegistry :many
...
-- name: AdminNavChangeOverview :many
...
-- name: AdminListNavChangeCheckpoints :many
SELECT registry.detector_key, registry.detector_version, registry.status,
...
-- name: AdminCountNavChangeEvents :one
...
WHERE (sqlc.arg(detector_key)::text='' OR event.detector_key=sqlc.arg(detector_key))
  ...
  AND (sqlc.arg(entity_id)::bigint=0 OR event.site_id=sqlc.arg(entity_id));

-- name: AdminListNavChangeEvents :many
