-- name: AdminGameCollectionCounts :one
SELECT
...
-- name: AdminGameCollectionClock :one
SELECT now()::timestamptz AS control_now;

-- name: AdminListGameCollectorInstances :many
WITH ranked AS (
    SELECT instance.*,
...
-- name: AdminCountGameCollectorInstances :one
WITH ranked AS (
...
-- name: AdminListGameCollectionSchedules :many
SELECT schedule.*,
...
-- name: AdminGetGameCollectionSchedule :one
SELECT * FROM gfg_collection_schedules WHERE id = sqlc.arg(id);

-- name: AdminUpdateGameCollectionSchedule :one
...
-- name: AdminInsertGameManualJob :one
INSERT INTO gfg_collection_jobs (
    ...
    sqlc.arg(tasks), 200, 'steam', 'queued', sqlc.arg(requested_by),
    sqlc.narg(dedupe_key), now(), now()
)
...
-- name: EnqueueGameEntityCollectionJob :execrows
INSERT INTO gfg_collection_jobs (
    job_key, trigger, scope_type, scope_id, tasks, priority, concurrency_key,
    status, requested_by, dedupe_key, created_at, updated_at
) VALUES (
    'game.metadata', sqlc.arg(trigger), 'game', sqlc.arg(game_id)::bigint,
    ARRAY['details', 'news']::text[], 300, 'steam', 'queued',
    sqlc.arg(requested_by),
    'game:' || (sqlc.arg(game_id)::bigint)::text || ':metadata:' || (sqlc.arg(appid)::bigint)::text,
    now(), now()
)
ON CONFLICT (dedupe_key)
    WHERE dedupe_key IS NOT NULL AND status IN ('queued', 'running')
...
-- name: AdminListGameCollectionJobs :many
...
-- name: AdminCountGameCollectionRuns :one
SELECT count(*)::bigint
...
-- name: AdminGetGameCollectionJob :one
SELECT * FROM gfg_collection_jobs WHERE id = sqlc.arg(id);

-- name: AdminCancelGameCollectionJob :one
...
-- name: AdminRetryGameCollectionJob :one
UPDATE gfg_collection_jobs
...
-- name: AdminListGameCollectionRuns :many
...
-- name: AdminCountGameCollectionResults :one
SELECT count(*)::bigint
FROM gfg_collection_task_results
WHERE run_id = sqlc.arg(run_id)
  AND (sqlc.narg(game_id)::bigint IS NULL OR game_id = sqlc.narg(game_id))
  AND (sqlc.narg(appid)::bigint IS NULL OR appid = sqlc.narg(appid));

-- name: AdminGetGameCollectionRun :one
SELECT run.*, job.job_key, job.trigger, job.scope_type, job.scope_id, job.target
FROM gfg_collection_runs run JOIN gfg_collection_jobs job ON job.id = run.job_id
WHERE run.id = sqlc.arg(id);

-- name: AdminListGameCollectionResults :many
SELECT * FROM gfg_collection_task_results
WHERE run_id = sqlc.arg(run_id)
  AND (sqlc.narg(game_id)::bigint IS NULL OR game_id = sqlc.narg(game_id))
  AND (sqlc.narg(appid)::bigint IS NULL OR appid = sqlc.narg(appid))
...
-- name: AdminListGameCollectionChartRows :many
...
FROM gfg_collection_jobs job
LEFT JOIN LATERAL (
    SELECT * FROM gfg_collection_runs candidate
...
-- name: AdminGameCollectionTargetExists :one
SELECT EXISTS (SELECT 1 FROM gfg_game WHERE id = sqlc.arg(id))::boolean;
