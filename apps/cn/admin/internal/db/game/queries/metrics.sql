-- name: AdminListGameMetricRegistry :many
...
-- name: AdminListGameMetricCheckpoints :many
...
-- name: AdminGameMetricOverview :many
...
-- name: AdminListGameMetricDaily :many
...
-- name: AdminCountGameMetricDaily :one
SELECT count(*)::bigint
FROM gfg_metric_daily
WHERE (sqlc.arg(metric_key)::text = '' OR metric_key = sqlc.arg(metric_key))
  AND (sqlc.arg(metric_version)::integer = 0 OR metric_version = sqlc.arg(metric_version))
...
-- name: AdminCountGameMetricEntities :one
SELECT count(*)::bigint
...
-- name: AdminListGameMetricEntities :many
