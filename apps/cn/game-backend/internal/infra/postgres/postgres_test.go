...
func TestParseConfigDefaults(t *testing.T) {
	config, pingTimeout, err := ParseConfig(Config{ConnectionString: "postgres://user:pass@localhost:5432/gfg?sslmode=disable"}, "gofurry-game-backend")
	if err != nil {
		t.Fatal(err)
	}
	...
	if got := config.ConnConfig.RuntimeParams["application_name"]; got != "gofurry-game-backend" {
