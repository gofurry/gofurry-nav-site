...
func TestDetailsMapperCanonicalReleasePrecisions(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if got.Precision != test.precision || got.RawText != test.raw || got.SourceRegion != domain.RegionUS || got.SourceLocale != domain.StoreLocaleEN || got.Normalizer != "steam-go/v1.3.12" {
