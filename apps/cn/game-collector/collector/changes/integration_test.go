...
func TestGameChangeProjectorsIntegration(t *testing.T) {
	...
	if err := tx.QueryRow(ctx, `INSERT INTO gfg_game_tracking_periods(game_id,appid,tracked_from,tracked_until,tracking_basis,opened_reason,closed_reason) VALUES(870001,870002,$1,$2,'legacy_observed','change_test','change_test') RETURNING id`, day4, day4.AddDate(0, 0, 2)).Scan(&period2); err != nil {
		t.Fatal(err)
	}
	for _, fixture := range []struct {
		day    time.Time
		period int64
	}{{day1, period1}, {day2, period1}, {day3, period1}, {day4, period2}} {
	...
	for _, metric := range []string{"free_game_share", "windows_support", "linux_support"} {
		for _, fixture := range []struct {
			day           time.Time
			state, reason string
