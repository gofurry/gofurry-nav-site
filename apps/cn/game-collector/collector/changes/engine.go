...
func (engine *Engine) Start(parent context.Context) error {
	...
	if err := engine.Reconcile(ctx); err != nil && !errors.Is(err, context.Canceled) {
		log.Error("startup Game change reconcile failed without stopping Collector: " + err.Error())
	}
	engine.wg.Add(1)
	go func() {
		...
		for {
			...
			case <-ticker.C:
				if err := engine.Reconcile(ctx); err != nil && !errors.Is(err, context.Canceled) {
					log.Error("Game change reconcile failed without stopping Collector: " + err.Error())
				}
			}
		}
	}()
...
func reconcileDetectors(detectors []registeredDetector, run func(Contract) (DayResult, error)) error {
	...
	for _, detector := range detectors {
		...
		if result.Processed {
			log.Info("Game changes finalized, detector=", result.DetectorKey, " version=", result.DetectorVersion, " projection_date=", result.ProjectionDate, " events=", result.EventCount)
		}
	}
	return resultErr
}
...
func (engine *Engine) Status(ctx context.Context) ([]Status, error) {
	...
	rows, err := gamesqlc.New(engine.pool).ListGameChangeCheckpoints(ctx)
	if err != nil {
		return nil, err
	}
	result := make([]Status, 0, len(rows))
	for _, row := range rows {
		result = append(result, Status{DetectorKey: row.DetectorKey, DetectorVersion: row.DetectorVersion, Status: row.Status,
			WatermarkPolicy: row.WatermarkPolicy, SourceStartDate: dateString(row.SourceStartDate), ProcessedThrough: dateString(row.ProcessedThrough),
			...
			LagDays: lagDays(row.SourceStartDate, row.ProcessedThrough, row.UpstreamProcessedThrough)})
	}
	return result, nil
}
...
func (engine *Engine) RunNext(ctx context.Context, key string, version int32, dryRun bool) (DayResult, error) {
	if _, ok := contractFor(key, version); !ok {
		return DayResult{}, fmt.Errorf("Game change detector is not compiled for %s", catalogID(key, version))
	}
	...
	cp, err := queries.LockGameChangeCheckpoint(ctx, gamesqlc.LockGameChangeCheckpointParams{DetectorKey: key, DetectorVersion: version})
	if err != nil {
		return DayResult{}, err
	}
	if cp.Status != "active" {
	...
	upstream, err := queries.GameChangeUpstreamProcessedThrough(ctx, gamesqlc.GameChangeUpstreamProcessedThroughParams{DetectorKey: key, DetectorVersion: version})
	if err != nil {
		return DayResult{}, err
	}
	...
	rows, err := queries.AdvanceGameChangeCheckpoint(ctx, gamesqlc.AdvanceGameChangeCheckpointParams{ProcessedThrough: pgDate(day), DetectorKey: key, DetectorVersion: version})
	if err != nil {
		return DayResult{}, err
	}
	if rows != 1 {
		return DayResult{}, fmt.Errorf("advance Game change checkpoint %s on %s: updated %d rows", catalogID(key, version), result.ProjectionDate, rows)
	}
...
func (engine *Engine) plan(ctx context.Context, contract Contract, from, through *time.Time, maxDays int) ([]DayResult, error) {
	...
	defer tx.Rollback(ctx)
	queries := gamesqlc.New(tx)
	cp, err := queries.LockGameChangeCheckpoint(ctx, gamesqlc.LockGameChangeCheckpointParams{DetectorKey: contract.Key, DetectorVersion: contract.Version})
	if err != nil {
		return nil, err
	}
	day, ok := nextDate(cp.SourceStartDate, cp.ProcessedThrough)
	...
	upstream, err := queries.GameChangeUpstreamProcessedThrough(ctx, gamesqlc.GameChangeUpstreamProcessedThroughParams{DetectorKey: contract.Key, DetectorVersion: contract.Version})
	if err != nil {
		return nil, err
	}
...
// Rebuild recomputes from start through the locked checkpoint because change
// detectors have previous-state memory. The checkpoint is never moved.
func (engine *Engine) Rebuild(ctx context.Context, key string, version int32, start time.Time, through *time.Time, maxDays int, dryRun bool) (Summary, error) {
	...
	for day := utcDate(start); !day.After(end); day = day.AddDate(0, 0, 1) {
		result := DayResult{DetectorKey: key, DetectorVersion: version, ProjectionDate: day.Format(time.DateOnly)}
		upstream, upstreamErr := queries.GameChangeUpstreamProcessedThrough(ctx, gamesqlc.GameChangeUpstreamProcessedThroughParams{DetectorKey: key, DetectorVersion: version})
		if upstreamErr != nil {
...
func (engine *Engine) nextDetectorDate(ctx context.Context, contract Contract) (time.Time, bool, error) {
	...
	defer tx.Rollback(ctx)
	cp, err := gamesqlc.New(tx).LockGameChangeCheckpoint(ctx, gamesqlc.LockGameChangeCheckpointParams{DetectorKey: contract.Key, DetectorVersion: contract.Version})
	if err != nil {
		return time.Time{}, false, err
	}
	day, ok := nextDate(cp.SourceStartDate, cp.ProcessedThrough)
...
func (engine *Engine) resolve(ctx context.Context, key string, version int32, activeOnly bool) ([]registeredDetector, error) {
	...
	if key != "" && len(result) == 0 {
		return nil, fmt.Errorf("Game change detector %s is not registered%s", catalogID(key, version), map[bool]string{true: " and active", false: ""}[activeOnly])
	}
	if key != "" && version == 0 && len(result) > 1 {
		return nil, fmt.Errorf("Game change detector %s resolves to multiple versions; specify --version", key)
	}
	return result, nil
}
