...
func (gf *goFurry) Serve(ctx context.Context) error {
	...
	retentionDays := env.GetServerConfig().Collector.V2.Retention.PlayerCountsDays
	gf.facts = facts.New(gf.pool, facts.Options{
		...
		PlayerRawAge:      time.Duration(retentionDays) * 24 * time.Hour,
	...
	gf.changes = changes.New(gf.pool, changes.Options{
		ReconcileInterval: env.GetServerConfig().Changes.ReconcileInterval(),
	})
	if err = gf.changes.Start(ctx); err != nil {
		return errors.Join(err, gf.Shutdown())
	}
	gf.health.MarkReady()
...
func (gf *goFurry) Shutdown() error {
	...
	gf.stopOnce.Do(func() {
		...
		if gf.changes != nil {
			...
			shutdownErr = errors.Join(shutdownErr, gf.changes.Shutdown(ctx))
			cancel()
			gf.changes = nil
		}
		if gf.metrics != nil {
