-- name: EnsureGameCollectionSchedule :exec
INSERT INTO gfg_collection_schedules (
    ...
    sqlc.arg(job_key), sqlc.arg(name), true, sqlc.arg(schedule_kind),
...
-- name: ListGameCollectionSchedules :many
SELECT *
FROM gfg_collection_schedules
ORDER BY job_key;

-- name: GameCollectionClock :one
SELECT now()::timestamptz AS control_now;

-- name: GetGameCollectionScheduleForUpdate :one
SELECT *
FROM gfg_collection_schedules
WHERE id = sqlc.arg(id)
FOR UPDATE;

-- name: UpdateGameCollectionScheduleCursor :exec
UPDATE gfg_collection_schedules
SET last_materialized_for = sqlc.narg(last_materialized_for),
    next_scheduled_for = sqlc.narg(next_scheduled_for),
    updated_at = now()
WHERE id = sqlc.arg(id)
  AND version = sqlc.arg(version);

-- name: InsertGameScheduledJob :execrows
INSERT INTO gfg_collection_jobs (
    ...
    sqlc.arg(requested_by), sqlc.narg(dedupe_key), now(), now(),
...
-- name: ClaimNextGameCollectionJob :one
WITH candidate AS (
    SELECT queued.id
...
SET status = 'running',
    claimed_by = sqlc.arg(instance_id),
...
-- name: SkipOverlappingGamePointInTimeJobs :execrows
...
INSERT INTO gfg_collection_runs (
    ...
    sqlc.arg(expected_count), 0, 0, 0, 0, 0,
    sqlc.arg(schedule_delay_ms), 0
)
RETURNING *;

-- name: RenewGameCollectionLease :execrows
UPDATE gfg_collection_jobs
SET lease_until = now() + sqlc.arg(lease_seconds)::bigint * interval '1 second',
    updated_at = now()
WHERE id = sqlc.arg(job_id)
  AND claimed_by = sqlc.arg(instance_id)
  AND status = 'running';

-- name: GameCollectionCancelRequested :one
...
-- name: GameCollectionLaneActive :one
SELECT EXISTS (
    SELECT 1
    FROM gfg_collection_jobs
    WHERE concurrency_key = sqlc.arg(concurrency_key)
      AND status = 'running'
)::boolean;

-- name: ListExpiredGameCollectionJobs :many
...
-- name: FailLostGameCollectionRun :execrows
...
-- name: FinalizeGameCollectionJob :execrows
UPDATE gfg_collection_jobs
...
-- name: InsertGameCollectionTaskResult :exec
INSERT INTO gfg_collection_task_results (
    ...
    sqlc.arg(run_id), sqlc.arg(task_type), sqlc.arg(game_id), sqlc.arg(appid),
    sqlc.arg(status), sqlc.arg(upstream_status_code), sqlc.arg(traffic_bucket),
    sqlc.arg(retry_count), sqlc.arg(duration_ms), sqlc.arg(error_kind),
...
-- name: RegisterGameCollectorInstance :exec
...
-- name: HeartbeatGameCollectorInstance :execrows
UPDATE gfg_collector_instances
SET last_heartbeat_at = now()
WHERE instance_id = sqlc.arg(instance_id) AND stopped_at IS NULL;

-- name: StopGameCollectorInstance :execrows
UPDATE gfg_collector_instances
SET last_heartbeat_at = now(), stopped_at = now()
WHERE instance_id = sqlc.arg(instance_id) AND stopped_at IS NULL;

-- name: GetGameTarget :one
SELECT id, appid
FROM gfg_game
WHERE id = sqlc.arg(id);

-- name: DeleteGameCollectionTaskResultsOlderThan :execrows
DELETE FROM gfg_collection_task_results
WHERE started_at < sqlc.arg(cutoff);
