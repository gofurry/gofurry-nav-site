...
func (engine *Engine) Start(parent context.Context) error {
	...
	if err := engine.Reconcile(ctx); err != nil && !errors.Is(err, context.Canceled) {
		log.Error("startup Nav metric reconcile failed without stopping Collector: " + err.Error())
	}
	engine.wg.Add(1)
	go func() {
		...
		for {
			...
			case <-ticker.C:
				if err := engine.Reconcile(ctx); err != nil && !errors.Is(err, context.Canceled) {
					log.Error("Nav metric reconcile failed without stopping Collector: " + err.Error())
				}
			}
		}
	}()
...
func (engine *Engine) Reconcile(ctx context.Context) error {
	...
	for _, metric := range metrics {
		...
		if result.Processed {
			log.Info("Nav metric finalized, metric=", result.MetricKey, " version=", result.MetricVersion, " fact_date=", result.FactDate)
		}
	}
	return reconcileErr
}
...
func (engine *Engine) Status(ctx context.Context) ([]Status, error) {
	...
	rows, err := navsqlc.New(engine.pool).ListNavMetricCheckpoints(ctx)
	if err != nil {
		return nil, err
	}
	result := make([]Status, 0, len(rows))
	for _, row := range rows {
		upstream := minWatermark(row.TargetProcessedThrough, row.SiteProcessedThrough)
		result = append(result, Status{
			...
			UpstreamProcessedThrough: dateString(upstream), LatestAvailableDay: dateString(upstream),
			LagDays: metricLagDays(row.SourceStartDate, row.ProcessedThrough, upstream),
		})
	}
	return result, nil
}
...
func (engine *Engine) RunNext(ctx context.Context, metricKey string, metricVersion int32, dryRun bool) (DayResult, error) {
	if _, ok := contractFor(metricKey, metricVersion); !ok {
		return DayResult{}, fmt.Errorf("Nav metric evaluator is not compiled for %s", catalogID(metricKey, metricVersion))
	}
	...
	checkpoint, err := queries.LockNavMetricCheckpoint(ctx, navsqlc.LockNavMetricCheckpointParams{MetricKey: metricKey, MetricVersion: metricVersion})
	if err != nil {
		return DayResult{}, err
	}
	...
	watermarks, err := queries.NavMetricUpstreamProcessedThrough(ctx)
	if err != nil {
		return DayResult{}, err
	}
	upstream := minWatermark(watermarks.TargetProcessedThrough, watermarks.SiteProcessedThrough)
	if !upstream.Valid || next.After(utcDate(upstream.Time)) {
		result.Reason = "upstream nav.target_facts/nav.site_facts watermarks are not ready"
		return result, tx.Commit(ctx)
	}
	result.PopulationEstimate, err = queries.CountNavMetricPopulation(ctx, pgDate(next))
	if err != nil {
		return DayResult{}, err
	}
	if dryRun {
	...
	if _, err := queries.ProjectNavMetricDay(ctx, navsqlc.ProjectNavMetricDayParams{
		MetricKey: metricKey, MetricVersion: metricVersion, FactDate: pgDate(next),
	}); err != nil {
		return DayResult{}, err
	}
	rows, err := queries.AdvanceNavMetricCheckpoint(ctx, navsqlc.AdvanceNavMetricCheckpointParams{
	...
	if rows != 1 {
		return DayResult{}, fmt.Errorf("advance Nav metric checkpoint %s on %s: updated %d rows", catalogID(metricKey, metricVersion), result.FactDate, rows)
	}
...
func (engine *Engine) rebuildDay(ctx context.Context, metricKey string, metricVersion int32, day time.Time, dryRun bool) (DayResult, error) {
	...
	if !upstream.Valid || day.After(utcDate(upstream.Time)) {
		return DayResult{}, fmt.Errorf("cannot rebuild %s: upstream Nav Fact watermarks are not ready", result.FactDate)
	}
	result.PopulationEstimate, err = queries.CountNavMetricPopulation(ctx, pgDate(day))
	if err != nil {
		return DayResult{}, err
	}
	if dryRun {
...
func (engine *Engine) plan(ctx context.Context, contract Contract, from, through *time.Time, maxDays int) ([]DayResult, error) {
	...
	defer tx.Rollback(ctx)
	queries := navsqlc.New(tx)
	checkpoint, err := queries.LockNavMetricCheckpoint(ctx, navsqlc.LockNavMetricCheckpointParams{MetricKey: contract.Key, MetricVersion: contract.Version})
	if err != nil {
		return nil, err
	}
	next, ok := nextDate(checkpoint.SourceStartDate, checkpoint.ProcessedThrough)
	...
	for {
		...
		if !upstream.Valid || next.After(utcDate(upstream.Time)) {
			day.Reason = "upstream nav.target_facts/nav.site_facts watermarks are not ready"
			result = append(result, day)
			break
		}
		day.PopulationEstimate, err = queries.CountNavMetricPopulation(ctx, pgDate(next))
		if err != nil {
			return nil, err
		}
		day.Reason = "dry-run"
...
func (engine *Engine) nextMetricDate(ctx context.Context, contract Contract) (time.Time, bool, error) {
	...
	defer tx.Rollback(ctx)
	checkpoint, err := navsqlc.New(tx).LockNavMetricCheckpoint(ctx, navsqlc.LockNavMetricCheckpointParams{MetricKey: contract.Key, MetricVersion: contract.Version})
	if err != nil {
		return time.Time{}, false, err
	}
	next, ok := nextDate(checkpoint.SourceStartDate, checkpoint.ProcessedThrough)
...
func (engine *Engine) resolveMetrics(ctx context.Context, metricKey string, metricVersion int32, activeOnly bool) ([]registeredMetric, error) {
	...
	for _, row := range rows {
		...
		if !compiled || (metricKey != "" && row.MetricKey != metricKey) || (metricVersion > 0 && row.MetricVersion != metricVersion) {
			continue
		}
	...
	if metricKey != "" && len(result) == 0 {
		return nil, fmt.Errorf("Nav metric %s is not registered or available for this operation", catalogID(metricKey, metricVersion))
	}
	if metricKey != "" && metricVersion == 0 && len(result) > 1 {
		return nil, fmt.Errorf("Nav metric %s resolves to multiple versions; specify --version", metricKey)
	}
	return result, nil
}

func minWatermark(first, second pgtype.Date) pgtype.Date {
	if !first.Valid || !second.Valid {
		return pgtype.Date{}
	}
	if utcDate(first.Time).Before(utcDate(second.Time)) {
		return pgtype.Date{Time: utcDate(first.Time), Valid: true}
	}
	return pgtype.Date{Time: utcDate(second.Time), Valid: true}
}
