...
func TestValidateConfig(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "loopback", config: Config{Enabled: true, ListenAddr: "127.0.0.1:19091"}},
		{name: "private", config: Config{Enabled: true, ListenAddr: "10.0.0.8:19091"}},
		{name: "missing address", config: Config{Enabled: true}, wantErr: true},
		{name: "public", config: Config{Enabled: true, ListenAddr: "8.8.8.8:19091"}, wantErr: true},
	} {
...
func TestEnabledServerShutsDown(t *testing.T) {
	server, err := New(Config{Enabled: true, ListenAddr: "127.0.0.1:19091"}, func(context.Context) error { return nil })
	if err != nil {
		t.Fatal(err)
	}
