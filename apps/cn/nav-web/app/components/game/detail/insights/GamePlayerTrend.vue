<template>
  <section class="game-insights-chart-card" data-player-history>
    <div class="entity-insights-heading">
      <div>
        <p class="entity-insights-eyebrow">{{ $t('insights.entity.dailyMaximum') }}</p>
        <h3>{{ $t('insights.entity.playerTrend') }}</h3>
      </div>
    </div>
...
async function renderChart() {
  ...
  const colors = {
    ...
    area: dark ? 'rgba(125, 211, 252, .16)' : 'rgba(217, 119, 6, .14)',
  }
  const series = props.points.map(point => ({ value: point.max, ...point }))
  ...
  chart.value.setOption({
    ...
    tooltip: {
      ...
      formatter(params: Array<{ data: GameInsightPlayerPoint & { value: number } }>) {
        const point = params?.[0]?.data
        if (!point) return ''
        const average = point.avg === null ? '—' : new Intl.NumberFormat(locale.value).format(point.avg)
        return `${point.date}<br/>${t('insights.entity.playerMax')}: ${new Intl.NumberFormat(locale.value).format(point.max)}<br/>${t('insights.entity.playerAverage')}: ${average}`
      },
    },
    xAxis: {
      type: 'category',
    ...
    series: [{
      name: t('insights.entity.playerMax'),
      type: 'line',
      data: series,
      connectNulls: false,
      symbol: 'circle',
