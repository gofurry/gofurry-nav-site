...
- **状态：** 已完成
- **范围：** 稳定性 / 运维 / 测试
- **目标：** 根据生产环境低风险验证结果，修补 Phase 0 暴露的小问题，并收敛采集日志保留规模。
...
- [x] 生产环境更新前验证索引现状，确认 `name, create_time DESC, id DESC` 组合索引已存在且有效。
- [x] 修正手动索引 SQL，避免不同索引名造成重复等价索引，并明确 `CREATE INDEX CONCURRENTLY` 不可在事务中执行。
- [x] 观察 Ping 间隔与实际耗时，确认防重入 skipped 日志符合预期。
- [x] 统计 HTTP 单目标失败原因，区分域名不存在、连接拒绝、超时和证书问题。
- [x] 观察 DNS 查询超时现象，暂不引入 multi-resolver，避免增加单人维护复杂度。
- [x] 根据日志样本将运行期日志切换为中文，并保持一行式人类可读格式。
- [x] 修复 Redis 空 Hash 写入问题，并增加回归测试。
- [x] 将生产采集日志保留数调整为够用即可：Ping 288 条、HTTP 240 条、DNS 60 条。

#### 验收标准

- 生产环境测试通过，三类协议采集流程能正常完成。
- 线上日志可以快速回答：哪类协议、哪个目标、哪个阶段失败。
- 大表清理不造成明显周期性峰值，日志保留规模符合当前业务需要。
- Phase 0 的 bug fix 不改变旧 Redis key、旧页面展示和旧接口读数。
- Redis 不再出现空 Hash 写入导致的 `ERR wrong number of arguments for 'hset' command`。

#### 备注

multi-resolver 只作为后续 DNS 可靠性观察方向保留，不在 Phase 0 默认引入。当前 DNS 仍使用单 resolver，避免把采集器做复杂。
...
- 继续观察单目标失败和 DNS timeout，不急于扩大协议能力。
- 只修确定性 bug，不引入影响后端/Nuxt 旧读取路径的新数据面。
