...
<template>
  <main class="space-y-5">
    <header class="page-header flex-col md:flex-row">
      <div>
        <h1 class="text-2xl font-semibold">采集观测</h1>
    ...
    <section class="grid gap-4 lg:grid-cols-[1fr_1.2fr]">
      ...
      <div class="workspace-section">
        ...
        <div class="mt-3 grid gap-2 sm:grid-cols-2 xl:grid-cols-3">
          <div v-for="item in status?.summary || []" :key="`${item.task_type}-${item.status}`" class="summary-cell">
    ...
    <section class="workspace-section">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          <input v-model="taskTypeFilter" class="ui-control px-3 py-2 text-sm" placeholder="task_type" />
          <select v-model="runStatusFilter" class="ui-control px-3 py-2 text-sm">
    ...
    <section class="workspace-section">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          <select v-model="resultStatusFilter" class="ui-control px-3 py-2 text-sm">
            <option value="">全部状态</option>
            <option value="success">success</option>
            <option value="partial">partial</option>
            <option value="failed">failed</option>
    ...
    <section class="workspace-section">
      <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div class="flex gap-2">
          <input v-model="gameID" class="ui-control px-3 py-2 text-sm" placeholder="game id" @keyup.enter="loadGameStatus" />
          <button class="ui-button px-3 py-2 text-sm" @click="loadGameStatus">查询</button>
        </div>
      </div>
      <div v-if="gameStatus" class="mt-4 grid gap-4 md:grid-cols-2">
        <div class="detail-block text-sm">
        ...
        <div class="detail-block text-sm">
          ...
          <div class="mt-2 flex flex-wrap gap-2">
            <span v-for="price in gameStatus.prices" :key="price.region" class="bg-[var(--control)] px-2 py-1" :class="price.available ? 'text-emerald-300' : 'text-[var(--text-muted)]'">
