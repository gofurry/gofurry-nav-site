...
`apps/cn/admin/react` is the sole Admin frontend. Its production build deterministically clears and writes `internal/transport/http/webui/dist` before the Go build embeds it. React owns content workspaces, Collection/Metrics/Changes, Workbench Attention, read-only Data Operations, Audit, and account governance, using the existing HTTP APIs and capability contract. Small sqlc-backed read models, one bounded PostgreSQL metadata service, and a capability-shaped Workbench aggregation assemble UI detail without introducing a BFF framework or alerting subsystem. See `contracts/admin-frontend.md`.
