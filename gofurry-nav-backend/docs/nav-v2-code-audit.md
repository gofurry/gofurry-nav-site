...
总体结论：v2 代码已经具备基础只读能力，未发现 P0 级别问题。审计发现的 P1/P2/P3 硬化项已在 `v0.4.1` 修复：target 归属边界已收紧，首次请求懒加载竞态已加锁保护，raw payload 已增加默认截断策略，detail 聚合已补 partial failure 语义与 HTTP contract 测试。
...
已把 active `gfn_collector_domain` 作为 target 分接口的授权来源。site summary 中的 target 仅作为详情页展示补充，并通过 `source`、`registered`、`summary_only` 标记，不能单独授权 raw/latest/history 分接口。
...
已使用锁保护 controller/service/DAO singleton 的懒加载写入。测试注入保留 package-level hook，并在测试后恢复。
...
已在公开 HTTP 层增加 payload 输出策略，默认 `payload_mode=preview`，超过 `nav_v2.raw_payload_preview_bytes` 时返回截断元数据，完整 payload 通过显式 `payload_mode=full` 返回。
...
v0.4.1 已在 detail 聚合内并发读取独立数据面，并增加 partial failure 策略，避免 trend/change/light probe 旁路失败拖垮整个 detail。
...
解析失败时已增加结构化日志，包含 `redis_key`、`site_id`、`target`、`protocol` 和原始错误摘要，不记录完整 payload。
...
已增加独立配置 `nav_v2.enabled`、`nav_v2.summary_enabled`、`nav_v2.detail_enabled`、`nav_v2.read_model_enabled`。保持向后兼容：旧 `summary_enabled=true` 会默认开启 summary/detail/read model 路由。
...
已补齐 controller table tests，覆盖 6 个 v2 路由的成功响应、参数错误、service error 透传和 URL encoded target。
...
1. `v0.4.1` 已收紧 target 归属校验，区分 DB registered target 与 summary-only target。
2. `v0.4.1` 已用锁保护新增 v2 singleton/DAO 懒加载写入。
3. `v0.4.1` 已为 raw payload 增加默认截断策略，并补大 payload 测试。
4. `v0.4.1` 已优化 detail 聚合读取路径，完成独立分支并发读取。
5. `v0.4.1` 已补齐日志、开关粒度和 controller HTTP contract 测试。
