...
建议保留现有 `/api/v1/game/*`，新增 `/api/v2/game/*`。v1 不立即删除，等前端 games v2 全部切换后再清理。
...
| `GET` | `/api/v2/game/sync/list` | RAG/搜索同步轻量列表 |
| `GET` | `/api/v2/game/sync/info` | RAG/搜索同步详情，不触发浏览量 |
| `GET` | `/api/v2/game/sync/news` | RAG/搜索同步新闻 |
| `GET` | `/api/v2/game/sync/creators` | RAG/搜索同步创作者 |
